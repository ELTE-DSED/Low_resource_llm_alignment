...
class AutoregressivePolicy(Policy):
    ...
    def _respond(
        ...
        # input_lengths = query_attn_masks.sum(dim=1)  # attention mask sums: input tokens count

        
# Slice out the generated part
        responses = sequences[:, queries.size(1):]  # assumes same padding for batch, adjust if variable


# Right-pad only the generated part to response_len
        # responses = right_pad(
        # generated,
    ...
    #     non_pad_mask = generated != self.base_tokenizer.pad_token_id
    #     first_non_pad_idx = non_pad_mask.float().argmax(dim=1)  # index of first non-pad per sequence

    # # Build new padded tensor with exactly response_len tokens
    #     responses = torch.full(
    #     (sequences.size(0), self.args.response_len),
    ...
    #         start_idx = first_non_pad_idx[i].item()
    #         gen_tokens = generated[i, start_idx : start_idx + self.args.response_len]
    #         responses[i, :gen_tokens.size(0)] = gen_tokens
